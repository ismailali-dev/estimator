...
@section('content')
    <div @class(['page-content', 'browse', 'container-fluid'])>
            ...
            <div @class(['col-md-12'])>
                <div @class(['panel', 'panel-bordered'])>
                    <div @class(['panel-body'])>
                            ...
                            <table id="dataTable" @class(['table', 'table-hover'])>
                                <thead>
                                    <tr>
                                        ...
                                        @endforeach
                                        <th @class(['actions', 'text-left', 'dt-not-orderable'])>{{ __('voyager::generic.actions') }}</th>
                                    </tr>
                                </thead>
                                <tbody>
                        ...
                        {{-- ✅ YOUR NEW VIEW ICON --}}
        <button class="btn btn-sm view-subscription" title="View"
            data-id="{{ $data->id }}"
        ...
        {{-- EXISTING ACTIONS (WITHOUT VIEW) --}}
        @foreach($actions as $action)
            @if (!method_exists($action, 'massAction') 
                && get_class($action) != 'TCG\Voyager\Actions\ViewAction')
...
/* Normalize all buttons (Voyager + custom) */
.bread-actions .btn,
.view-subscription {
    display: inline-flex;
    align-items: center;
...
.bread-actions .btn span {
    display: none;
}

/* Remove extra wrapper spacing */
.bread-actions .icon-action {
    margin: 0 !important;
    padding: 0 !important;
}
...
@section('javascript')
    ...
    console.log('User dddID:', user_id); // Debugging line

    $('#subscriptionModal').modal('show');
    $('#subscription-history').html('Loading...');

    $.get('/admin/get-user-subscriptions/'+ user_id, function (data) {
        $('#subscription-history').html(data);
    });
});
    </script>
@stop
